...
{
    public class AccountDetailsDataAdapter
    {
        private ErrorHandling _error;

        public AccountDetailsDataAdapter()
        {
        ...
        public AccountDetailModel GetAccountDetailsEntity(string AccountNo)
        {
            ...
            using (var db = new CodeTestContext())
            {
                ...
                if (query == null)
                {
                    _error.throwError("AccountNo does not exist", "AccountNo not found", HttpStatusCode.NotFound);
                }
